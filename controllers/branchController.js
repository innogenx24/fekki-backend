...
exports.updateBranchById = async (req, res) => {
    ...
    try {
      ...
      // Check if a branch with the same country, state, and city already exists (excluding the current branch)
      const branchExists = await Branch.findOne({
        where: {
          branch_name,
          role_id: existingBranch.role_id, // Ensure the check is done for the same role_id
          id: { [Sequelize.Op.ne]: id }, // Exclude the current branch being updated
        },
      });
      ...
      if (branchExists) {
        return res.status(400).json({
          success: false,
          message: 'Branch already exists with the same country, state, and city.',
        });
      }
